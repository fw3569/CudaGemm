...
#define BLOCK_SWIZZLE_STRIDE 2048
struct GemmSmem {
...
__global__ __launch_bounds__(THREADS, 2) void gemm_tf32_kernel(
  ...
  __half a_frag[2][IOWM][EOFA];
  __half b_frag[2][IOWN][EOFB];
  ...
  for (int k_tile = 0; k_tile < num_k_tiles; ++k_tile) {
    ...
    if (k_tile + 1 < num_k_tiles) {
      load_A_tile<aligned_block, aligned_16>(smem, nxt, A, bm,
                                             (k_tile + 1) * STRIDE_K, M, K);
      load_B_tile<aligned_block, aligned_16>(smem, nxt, B,
                                             (k_tile + 1) * STRIDE_K, bn, K, N);
    }
    cp_async_commit();
    int reg_cache_idx = 0;
    {
#pragma unroll
      for (int wi = 0; wi < IOWM; ++wi) {
        ...
        load_matrix_a(a_frag[reg_cache_idx][wi], &smem.A[cur][m_off][0],
                      SMEM_A_STRIDE, lane_id);
      }
#pragma unroll
      for (int wj = 0; wj < IOWN; ++wj) {
        int n_off = (wn * IOWN + wj) * EOIN;
        load_matrix_b(b_frag[reg_cache_idx][wj], &smem.B[cur][0][n_off],
...
#pragma unroll
        for (int wi = 0; wi < IOWM; ++wi) {
          int m_off = (wm * IOWM + wi) * EOIM;
          load_matrix_a(a_frag[reg_cache_idx][wi],
                        &smem.A[cur][m_off][k + EOIK], SMEM_A_STRIDE, lane_id);
        }
#pragma unroll
        for (int wj = 0; wj < IOWN; ++wj) {
          int n_off = (wn * IOWN + wj) * EOIN;
          load_matrix_b(b_frag[reg_cache_idx][wj],
                        &smem.B[cur][k + EOIK][n_off], SMEM_B_STRIDE, lane_id);
        }
      }
      reg_cache_idx ^= 1;
      for (int wi = 0; wi < IOWM; ++wi) {
#pragma unroll
        for (int wj = 0; wj < IOWN; ++wj) {
          mma(acc[wi][wj], a_frag[reg_cache_idx][wi], b_frag[reg_cache_idx][wj],
              acc[wi][wj]);
        }
      }
    }
